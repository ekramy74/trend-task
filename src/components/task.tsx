...
export const Tasks = () => {
  ...
  return (
      <div className={"mt-2 px-20 py-10"}>
          ...
          <div className={"grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-5 select-none"}>
              {filteredTasks.map((item, index) => (
                  <div
                      className={"bg-white relative p-5 rounded-xl shadow-md transition-all ease-in-out duration-300 hover:-translate-y-2"}
                      key={item?.id + index}>
                      <div className={"absolute top-4 right-5 cursor-pointer"}
                           onClick={() => dispatch(changeTaskStatus(item.id))}>
                          {item.status === "uncompleted" ? <FaRegCircle className={"text-black"} size={24}/> :
